...
export default function AdminRecruiters() {
  ...
  const columns: Column<AdminUser>[] = [
    ...
    },
    {
      ...
      render: (_, user) => (
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-medium">{user.company?.name || 'Non renseigné'}</div>
            {user.company?.industry && (
              <div className="text-xs text-gray-500">{user.company.industry}</div>
            )}
          </div>
        </div>
      ),
    },
    {
      ...
      render: (_, user) => (
        <div>
          <div className="text-sm">{user.profile?.email || 'Non renseigné'}</div>
    ...
    },
    {
      ...
      render: (_, user) => (
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">offres publiées</div>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Gestion des Recruteurs</h1>
          <p className="text-muted-foreground">
            Gérez les comptes recruteurs et entreprises
          </p>
        </div>
        <Button>
      ...
      {data && (
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Avec Entreprise</CardTitle>
              <Building2 className="h-4 w-4 text-blue-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">
                {data.data.filter(u => u.company?.name).length}
              </div>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">
                {data.data.reduce((sum, u) => sum + (u.jobOffers?.length || 0), 0)}
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {data ? `${data.total} recruteur(s) trouvé(s)` : 'Chargement...'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
